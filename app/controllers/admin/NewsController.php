...
class NewsController extends AdminController {
	...
	public function store()
	{
		...
		if($validator->fails()) {
        	...
        	//create news
        	$inputNews = Input::only('type_new_id', 'title', 'description','start_date', 'sapo');
	...
	public function createHtmlPageNews($inputNew)
	{
    	...
    	$filePath = $viewPath.'/'.'news_news_'.$inputNew->slug.'_mobile.blade.php';
    	file_put_contents($filePath, $html);

    	$html = View::make('site.News.showNews_pc')->with(compact('inputNew'))->render();
    	$filePath = $viewPath.'/'.'news_news_'.$inputNew->slug.'_pc.blade.php';
    	file_put_contents($filePath, $html);
