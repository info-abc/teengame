...
class AccountController extends SiteController {
	...
	public function store()
	{
		...
		if($validator->fails()) {
        	...
        	if($id) {
        		return Redirect::action('SiteController@login')->with('message', 'Your account has been created!');
        	} else {
        		dd('Error');
        	}
        }
	...
	public function doAccount()
	{
		...
		if($validator->fails()) {
    		...
    		return Redirect::action('AccountController@account')->with('message', 'Update success');
        }
	}
